...
vec3 varyHue(vec2 coord, int iterations, int j, vec4 zx, vec4 zy) {
  float falloff = .996 + 0.06 * rand(coord + zx.x + zy.y);
  vec3 hsv = vec3(hue, 0.25, 0.1);

  if (j < iterations) {
    float co = (float(j) + 1. - log(log(length(vec2(zx.x, zy.x))))/log(2.));
...
vec3 mandelbrotIterateToColor(vec2 coord) {
  ...
  vec4 qs_tx = vec4(coord.x, vec3(0.));         // get position of current pixel
  vec4 qs_ty = vec4(coord.y, vec3(0.));

 // initialize complex variable with respect to current position, zoom, ...
  vec4 cx = qs_add(
    vec4(center.x, centerD.x, 0., 0.),
    qs_mul(
      qs_tx,
      vec4(zoom, 0., 0., 0.)
    )
  ...
  vec4 zx = cx;
  vec4 zy = cy;
  vec4 two = vec4(2.0, vec3(0.));j
  int j = iterations;

  vec4 e_radius = vec4(4., vec3(0.));   // no sqrt available so compare with radius^2 = 2^2 = 2*2 = 4
  ...
  return varyHue(coord, iterations, j, zx, zy);
}
