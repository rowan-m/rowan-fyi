...
class ThreeRenderer {
    ...
    initScene() {
        ...
        this.camera = new THREE.OrthographicCamera(
            fSize * this.width / -2, fSize * this.width / 2,
            fSize * this.height / 2, fSize * this.height / -2,
            0.1, 1000
        );
    ...
    buildAtom(atom) {
        ...
        for (let n = 0; n < nTotal; n++) {
            ...
            if ((Math.random() >= .5 && pBudget > 0) || nBudget <= 0) {
                // protons
            ...
            particle.position.x = x + (Math.random() - .5) * .2 * radius;
            particle.position.y = y + (Math.random() - .5) * .2 * radius;
            particle.position.z = z + (Math.random() - .5) * .2 * radius;
        ...
        this.electrons = [];
        const orbitMat = new THREE.MeshStandardMaterial({ color: 0xaabbaa, transparent: true, opacity: 0.1 });
        ...
        atom.electrons.forEach(e => {
            ...
            // orbits
            const orbitGeo = new THREE.TorusGeometry(e.w * 200, .1, 3, 180);
